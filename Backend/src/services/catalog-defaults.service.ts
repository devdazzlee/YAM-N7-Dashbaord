...
async function maxNumericCodeFromRows(codes: { code: string }[]): Promise<string> {
    let max = 999;
...
async function generateSupplierCode(tx: Tx): Promise<string> {
...
export const catalogDefaults = {
    async ensureDefaultSupplier(tx: Tx, excludeId?: string): Promise<string> {
    ...
    async ensureDefaultCategory(tx: Tx, excludeId?: string): Promise<string> {
        ...
        const created = await tx.category.create({
            data: {
                ...
                slug: DEFAULT_CATEGORY_SLUG,
                code: await maxNumericCodeFromRows(
    ...
    async ensureDefaultSubcategory(tx: Tx, excludeId?: string): Promise<string> {
        ...
        const created = await tx.subcategory.create({
            data: {
                ...
                code: await maxNumericCodeFromRows(
                    await tx.subcategory.findMany({ select: { code: true } }),
                ),
                is_active: true,
                display_on_pos: false,
            },
    ...
    async ensureDefaultUnit(tx: Tx, excludeId?: string): Promise<string> {
        ...
        const created = await tx.unit.create({
            data: {
                name: DEFAULT_GENERIC_NAME,
                code: await maxNumericCodeFromRows(
                    await tx.unit.findMany({ select: { code: true } }),
                ),
                is_active: true,
                display_on_pos: false,
            },
    ...
    async ensureDefaultBrand(tx: Tx, excludeId?: string): Promise<string> {
        ...
        const created = await tx.brand.create({
            data: {
                name: DEFAULT_GENERIC_NAME,
                code: await maxNumericCodeFromRows(
                    await tx.brand.findMany({ select: { code: true } }),
                ),
                is_active: true,
                display_on_pos: false,
            },
    ...
    async ensureDefaultColor(tx: Tx, excludeId?: string): Promise<string> {
        ...
        const created = await tx.color.create({
            data: {
                name: DEFAULT_GENERIC_NAME,
                code: await maxNumericCodeFromRows(
                    await tx.color.findMany({ select: { code: true } }),
                ),
                is_active: true,
                display_on_pos: false,
            },
    ...
    async ensureDefaultSize(tx: Tx, excludeId?: string): Promise<string> {
        ...
        const created = await tx.size.create({
            data: {
                name: DEFAULT_GENERIC_NAME,
                code: await maxNumericCodeFromRows(
                    await tx.size.findMany({ select: { code: true } }),
                ),
                is_active: true,
                display_on_pos: false,
            },
...
export const catalogDeleteOptions = {
    maxWait: 30000,
    timeout: 120000,
} as const;
