...
const Sizes: React.FC = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await apiClient.delete(`${API_BASE}/sizes/${deleteTarget.id}`);
      toast({
        title: "Success",
        description: res.data?.message || "Size deleted successfully",
      });
  ...
  const totalLinkedProducts = sizes.reduce((sum, s) => sum + (s.product_count || 0), 0);

  const filtered = sizes.filter(s => {
    const matchesSearch = s.name.toLowerCase().includes(search.toLowerCase()) || s.code.includes(search);
    ...
    <div className="p-4 md:p-6 space-y-4 md:space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-gradient-to-br from-indigo-50/40 to-white border-indigo-100/50 shadow-sm">
          <CardContent className="p-5 flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-xs font-bold text-indigo-600 uppercase tracking-wider">Total Sizes</p>
              <p className="text-3xl font-extrabold text-gray-900">{totalCount}</p>
              <p className="text-[10px] text-gray-500">Sizes registered in master</p>
            </div>
        ...
        <Card className="bg-gradient-to-br from-emerald-50/40 to-white border-emerald-100/50 shadow-sm">
          <CardContent className="p-5 flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-xs font-bold text-emerald-600 uppercase tracking-wider">Active Sizes</p>
        ...
        <Card className="bg-gradient-to-br from-rose-50/40 to-white border-rose-100/50 shadow-sm">
          <CardContent className="p-5 flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-xs font-bold text-rose-600 uppercase tracking-wider">Inactive Sizes</p>
              <p className="text-3xl font-extrabold text-gray-900">{inactiveCount}</p>
              <p className="text-[10px] text-gray-500">Disabled or hidden sizes</p>
            </div>
                  ...
                  <TableBody>
                    {filtered.map(s => (
                      <TableRow key={s.id} className="hover:bg-gray-50">
                        <TableCell className="font-mono font-medium text-gray-900">{s.code}</TableCell>
                        ...
                        <TableCell className="text-gray-600">{s.created_at.split('T')[0]}</TableCell>
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-2">
                            <Button size="sm" variant="outline" onClick={() => openDetail(s)} title="View Details">
                              <Eye className="h-4 w-4 text-gray-500"/>
                            </Button>
                            <Button size="sm" variant="outline" onClick={() => openEdit(s)} title="Edit Size">
                              <Edit className="h-4 w-4 text-indigo-600"/>
                            </Button>
                            <Button size="sm" variant="outline" className="border-rose-100 hover:bg-rose-50" onClick={() => setDeleteTarget(s)} title="Delete Size">
      ...
      <Dialog open={addOpen || editOpen} onOpenChange={(open) => { if (!open) { setAddOpen(false); setEditOpen(false); } }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{current ? 'Edit Size' : 'Create Size'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-5 py-2">
            <div>
              ...
              <Input 
                ...
                placeholder="Enter size name (e.g. Medium, Large, 500g)"
            ...
            <div className="flex items-center justify-between p-3 rounded-lg border border-gray-100 bg-gray-50/50">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-gray-500">Enable or disable this size in system workflows</p>
              </div>
            ...
            >
              {current ? 'Update Size' : 'Create Size'}
            </LoadingButton>
          </div>
        </DialogContent>
      </Dialog>
          ...
          {current && (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 border-b border-gray-100 pb-3">
                <div>
                  <span className="text-xs text-gray-500 uppercase tracking-wider block font-semibold">Size Code</span>
                ...
                <div>
                  <span className="text-xs text-gray-500 uppercase tracking-wider block font-semibold">Size Name</span>
