...
const Units: React.FC = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await apiClient.delete(`${API_BASE}/units/${deleteTarget.id}`);
      toast({
        title: "Success",
        description: res.data?.message || "Unit deleted successfully",
      });
      setDeleteTarget(null);
      fetchUnits();
    } catch (err: any) {
      const errMsg = extractApiError(err, "Failed to delete unit");
      toast({
        title: "Error",
        description: errMsg,
        variant: "destructive",
  ...
  const totalLinkedProducts = units.reduce((sum, u) => sum + (u.product_count || 0), 0);

  const filtered = units.filter(u => {
    const matchesSearch = u.name.toLowerCase().includes(search.toLowerCase()) || u.code.includes(search);
    ...
    <div className="p-4 md:p-6 space-y-4 md:space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-gradient-to-br from-indigo-50/40 to-white border-indigo-100/50 shadow-sm">
          <CardContent className="p-5 flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-xs font-bold text-indigo-600 uppercase tracking-wider">Total Units</p>
              <p className="text-3xl font-extrabold text-gray-900">{totalCount}</p>
              <p className="text-[10px] text-gray-500">Units registered in master</p>
            </div>
        ...
        <Card className="bg-gradient-to-br from-emerald-50/40 to-white border-emerald-100/50 shadow-sm">
          <CardContent className="p-5 flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-xs font-bold text-emerald-600 uppercase tracking-wider">Active Units</p>
        ...
        <Card className="bg-gradient-to-br from-rose-50/40 to-white border-rose-100/50 shadow-sm">
          <CardContent className="p-5 flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-xs font-bold text-rose-600 uppercase tracking-wider">Inactive Units</p>
              <p className="text-3xl font-extrabold text-gray-900">{inactiveCount}</p>
              <p className="text-[10px] text-gray-500">Disabled or hidden units</p>
            </div>
                  ...
                  <TableBody>
                    {filtered.map(u => (
                      <TableRow key={u.id} className="hover:bg-gray-50">
                        <TableCell className="font-mono font-medium text-gray-900">{u.code}</TableCell>
                        ...
                        <TableCell>
                          <span className="font-mono text-xs bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded border border-gray-200">
                            {u.product_count} product{u.product_count === 1 ? "" : "s"}
                          </span>
                        </TableCell>
                        <TableCell>
                          {u.is_active ? (
                        ...
                        <TableCell className="text-gray-600">{u.created_at.split('T')[0]}</TableCell>
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-2">
                            <Button size="sm" variant="outline" onClick={() => openDetail(u)} title="View Details">
                              <Eye className="h-4 w-4 text-gray-500"/>
                            </Button>
                            <Button size="sm" variant="outline" onClick={() => openEdit(u)} title="Edit Unit">
                              <Edit className="h-4 w-4 text-indigo-600"/>
                            </Button>
                            <Button size="sm" variant="outline" className="border-rose-100 hover:bg-rose-50" onClick={() => setDeleteTarget(u)} title="Delete Unit">
      ...
      <Dialog open={addOpen || editOpen} onOpenChange={(open) => { if (!open) { setAddOpen(false); setEditOpen(false); } }}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-5 py-2">
            <div>
              <Label htmlFor="unit-name">Name<span className="text-red-500 ml-1">*</span></Label>
              <Input
                id="unit-name"
                value={formName}
            ...
            <div className="flex items-center justify-between p-3 rounded-lg border border-gray-100 bg-gray-50/50">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-gray-500">Enable or disable this unit in system workflows</p>
              </div>
            ...
            >
              {current ? 'Update Unit' : 'Create Unit'}
            </LoadingButton>
          </div>
        </DialogContent>
      </Dialog>
          ...
          {current && (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 border-b border-gray-100 pb-3">
                <div>
                  <span className="text-xs text-gray-500 uppercase tracking-wider block font-semibold">Unit Code</span>
                ...
                <div>
                  <span className="text-xs text-gray-500 uppercase tracking-wider block font-semibold">Unit Name</span>
          ...
          <div className="flex flex-col items-center text-center space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <div className="text-sm text-gray-600 space-y-2">
                <p>
                  You are about to permanently delete the unit <strong className="text-rose-600 font-bold">"{deleteTarget?.name}"</strong> (Code: {deleteTarget?.code}).
                </p>
                <p className="bg-amber-50 border border-amber-200 text-amber-800 p-2.5 rounded text-xs text-left">
                  Linked products will be moved to the default unit (&quot;General&quot;). Products are not deleted. This action cannot be undone.
                </p>
              </div>
            </div>
